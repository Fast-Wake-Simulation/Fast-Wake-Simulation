...
public class DVM : MonoBehaviour {
    ...
    public Vector3[,,] elementPositions = new Vector3[555, 555, 2];
	...
	public float maxVorticityX = 0.0f;
	public float maxVorticityY = 0.0f;
	public float maxSineMultiplier = 0.0f;
	public float noiseGenerator;
    public float horseShoeVortexGenerator;
	...
	void Start () {
        ...
        //testing variables (THIS SHOULD BE ClEARED A LOT)
        initialPoint[0] = new Vector3(-0.0f, 0f, -5.3f);
        endPoint[0] = new Vector3(0.0f, 0.0f, 5.3f);
        initialPoint[1] = new Vector3(0.0f, -1f, -0.5f);
        endPoint[1] = new Vector3(0.0f, 1f, -0.5f);
    ...
    void spawnNewRow()
    {
        ...
        for (int x = 0; x < ElementSpanCountX; x++)
        {
            ...
            for (int sheetNo = 0; sheetNo < numberOfSheets; sheetNo++)
            {
                ...
                //Set the correct bluff body and noise vorticity
                //elementVorticities[x, 0, sheetNo] = new Vector2(0.0f * Mathf.Sin(rand.Next()) * 0.5f, 5.5f * Mathf.Sin(Time.time) * Mathf.Pow(0.5f, 2));
				elementVorticities[x, 0, sheetNo] = new Vector2(maxVorticityX * Mathf.Sin(Time.time * maxSineMultiplier) + (rand.Next(-5,5) * noiseGenerator), maxVorticityY * Mathf.Sin(Time.time * maxSineMultiplier)+ (rand.Next(-5,5) * noiseGenerator));

                //Set the correct horseshoe vortex shape
                if (x == 1) { elementVorticities[x, 0, sheetNo].y += -horseShoeVortexGenerator * (rb.velocity.magnitude / 25.0f); };
                if (x == ElementSpanCountX - 2) { elementVorticities[x, 0, sheetNo].y += horseShoeVortexGenerator * (rb.velocity.magnitude / 25.0f); };
                ...
                for (int y = 0; y < 20; y++)
                {
                    ...
                    for (int xi = 0; xi < 10; xi++)
                    {
                        for (int yi = 0; yi < 40; yi++)
                        {
							...
							for (int iterating = 0; iterating < numberOfSheets; iterating++){
                            ...
                            if (x != xi && y != yi)
                            {
                                ...
                                //First the radius,
                                Vector3 radius = elementPositions[x, y, iteration] - elementPositions[xi, yi, iterating];

                                //now the cumulative velocity fields can be superimposed
                                elementVelocities[x, y, iteration] += (1f / (4f * Mathf.PI)) * (Vector3.Cross(elementSheetwiseVorticity[xi, yi, iterating], radius) / (Mathf.Pow(radius.magnitude, 2)));
								//Debug.Log("Convecan");
                            }
            ...
            for (int x = 0; x < 10; x++)
            {
                for (int y = 0; y < ElementSpanCountY; y++)
                {
                    ...
                    else
                    {
						...
						elementPositions[x, y, iteration] += freeStreamVelocity;
