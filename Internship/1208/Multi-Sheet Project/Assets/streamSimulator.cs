...
public class streamSimulatorCopy : MonoBehaviour {
    ...
    void Start () {
        ...
        //sheetSpawnStartPoints[1] = new Vector3(-0.5f, -0.5f, -0.5f);
        //sheetSpawnEndPoints[1] = new Vector3(-0.5f, -0.5f, 0.5f);
    ...
    void applyInitialVorticityOfElements()
    {
        ...
        for (int sheetNo = 0; sheetNo < numberOfSheets; sheetNo++)
        {
            ...
            for (int x = 0; x < sheetSpanx; x++)
            {
                ...
                float xSine = xVorticityMagnitude * Mathf.Pow(Mathf.Sin((Time.time * sineMultiplier) + 0 / 2), 2);
