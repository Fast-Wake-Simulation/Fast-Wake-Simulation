...
{
    //This class is used in the setup stage of the aircraft, allowing the user to customise the aircraft, and see the results in realtime.

    // public  Vector3  velocityEarth =  new Vector3 (0, 0, 1); UNUSED
    public float allUpMass = 2.0f; // mass of whole aircraft in Kg
    public static float wingloading; // = 20; // N/m^2

    //public float thrustToWeight; // overall thrust to weight ratio

    public static float staticMargin = 0.1f; // (xac-xcg)/cbar dimensionless

    //Configurable aircraft ratios and dimensions. These must be static so that the results can persist into the flying and testing scene.
    public static float wingAspectRatio; //7f;
    public static float wingDihedral; //0f; //wing dihedral angle (degrees)
    public static float tailAspectRatio; //5f;
    public static float wingTailAreaRatio; //= 0.2f; // ratio of tail area to wing area. normally around 5.
    public static float tailVolumeRatio; //= 0.4f; // normally around 0.5
    ...
    //The sliders and text contained within the setup UI
    public Slider wingARSlider;
    public Text wingARSliderValText;
    public Slider wingLoadingSlider;
    public Text wingLoadingSliderText;
    public Slider tailARSlider;
    public Text tailARSliderValText;
    public Slider tailVRSlider;
    public Text tailVRSliderText;
    public Slider wingTailARSlider;
    public Text wingTailARSliderText;
	public Slider staticMarginSlider;
	public Text staticMarginSliderText;

    void Awake()
    {
        //Everytime the setup scene is loaded, these are the default values which the aircraft is set to. 
        wingAspectRatio = 7f; 
		wingloading = 20f;
        tailAspectRatio = 5f;
        wingTailAreaRatio = 0.2f;
        tailVolumeRatio = 0.4f;
		staticMargin = 0.1f;
        Time.timeScale = 0; //Freezes the scene so that the aircraft doesn't fly away while the user is customising it.
    }

	void Update ()
    {
        //Sets the aircraft ratios and dimensions to the values of the setup sliders.
        wingAspectRatio = wingARSlider.value;
        wingloading = wingLoadingSlider.value;
        tailAspectRatio = tailARSlider.value;
        wingTailAreaRatio = wingTailARSlider.value;
        tailVolumeRatio = tailVRSlider.value;
		staticMargin = staticMarginSlider.value;


        //Sets the slider value text to the values of the sliders. ToString("F1") rounds the values to 1 decimal place.
        wingARSliderValText.text = wingARSlider.value.ToString("F1");
        wingLoadingSliderText.text = wingLoadingSlider.value.ToString("F1");
        tailARSliderValText.text = tailARSlider.value.ToString("F1");
        wingTailARSliderText.text = wingTailARSlider.value.ToString("F1");
        tailVRSliderText.text = tailVRSlider.value.ToString("F1");
		staticMarginSliderText.text = staticMargin.ToString ("F1");

        //Allows the user to 'zoom in' to the aircraft using the mouse scrollwheel to control the camera's fov.
        Camera.main.fieldOfView = Camera.main.fieldOfView - Input.GetAxis("Mouse ScrollWheel") * 10f;
        Camera.main.fieldOfView = Mathf.Clamp(Camera.main.fieldOfView, 30, 80); //limits the maximum and minimum values of the camera's fov

        Generate(); //re-generates the aircraft using the values set by the onscreen setup sliders
    }

    void Generate () //Generates the aircraft using the values set by the onscreen setup sliders
    {
        ...
        //Creates joints between all of the aircraft's parts.
    ...
    //functions that are used by the setup menu buttons.
    public void Reset ()
    {
        SceneManager.LoadScene("Menu"); //reloads the setup scene
    }

    public void Test ()
    {
        Time.timeScale = 1; //Un-freezes time
    ...
    {
        Time.timeScale = 1; //Un-freezes time
        SceneManager.LoadScene("Main"); //loads the main flying arena
    }
}
